...
test('build publishes only runtime files, removes stale output and has a reproducible portable manifest',async()=>{
 ...
 try{
  ...
  for(const [name,body] of Object.entries({'index.html':'<html></html>','src/app.js':'export const ok=true;','public/asset.txt':'asset','vendor/runtime.js':'export {};','LICENSE':'license','ASSET_LICENSES.md':'attribution','CHANGELOG.md':'changes','README.md':'developer instructions','docs/HISTORY.md':'history','docs/KNOWN_ISSUES.md':'limits','docs/internal.md':'internal','tests/private.txt':'test','dist/stale.txt':'old release','.local/private.txt':'private','package.json':'{"version":"0.4.0","type":"module"}'}))await writeFile(join(root,name),body);
  const build=()=>execFileSync(process.execPath,[join(root,'tools/build.mjs')],{cwd:tmpdir(),stdio:'pipe'});
  build();
  assert.deepEqual((await readdir(join(root,'dist'))).sort(),['.nojekyll','ASSET_LICENSES.md','CHANGELOG.md','LICENSE','build-manifest.json','docs','index.html','public','src','vendor'].sort());
  assert.deepEqual((await readdir(join(root,'dist/docs'))).sort(),['HISTORY.md','KNOWN_ISSUES.md']);
