...
test('English menu and settings are translated with a language selector',()=>{const l=requireLocale();l.setLanguage('en');try{assert.match(screen('main'),/Start demo mission/);assert.match(screen('settings'),/Language/);assert.match(screen('settings'),/data-setting="language"/);assert.doesNotMatch(screen('main'),/Rozpocznij|Ustawienia|Pęknięta/);}finally{l.setLanguage('pl');}});
