...
class Application {
 constructor(){this.canvas=document.querySelector('#game');this.state='main';this.returnState='main';this.world=null;this.view=null;this.loadGeneration=0;this.checkpoint=null;this.storageWarning='';this.errorMessage='';this.lockMessage='';this.store=new Store(message=>{this.storageWarning=message;this.ui?.toast(message,9);});this.settings=this.store.settings();this.audio=new BattlefieldAudio(this.settings);this.clock=new FixedClock();this.ui=new UI(this);this.input=new Input(this.canvas,this.settings,{pause:()=>this.pause(),debug:()=>{this.ui.debug=!this.ui.debug;},lockError:()=>this.lockError()});this.lastFrame=performance.now();this.performance=new PerformanceMonitor();this.metrics=this.performance.metrics;this.frame=0;this.alive=true;
  ...
  window.ZiemiaNiczyja={get state(){return app.state;},inspect:()=>this.inspect(),version:'0.3.0'};
 }
 go(state){this.state=state;const active=state==='playing';this.input.setActive(active);this.clock.reset();this.performance?.reset();if(this.performance)this.metrics=this.performance.metrics;if(!active)this.view?.setDiagnosticsEnabled(false);this.lastFrame=performance.now();if(!active){this.audio.pause();if(document.pointerLockElement===this.canvas)document.exitPointerLock();}this.ui.render(state);}
 ...
 changeSetting(key,value){this.settings[key]=value;this.input.settings=this.settings;this.audio.setSettings(this.settings);this.store.saveSettings(this.settings);}
 ...
 loop(now){
  ...
  if(this.state==='playing'&&this.world&&this.view){
   try{
    ...
    const simulation=performance.now()-simulationStart,renderStart=performance.now();
    ...
    this.performance.record({frame:delta*1000,cpu:performance.now()-started,simulation,render},now);
    this.metrics=this.performance.metrics;
