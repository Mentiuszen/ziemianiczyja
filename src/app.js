...
export class Application {
 constructor(){this.canvas=document.querySelector('#game');this.state='main';this.returnState='main';this.world=null;this.view=null;this.loadGeneration=0;this.enterSequence=0;this.enterPending=false;this.actionSequence=0;this.checkpoint=null;this.storageWarning='';this.errorMessage='';this.lockMessage='';this.store=new Store(message=>{this.storageWarning=message;this.ui?.toast(message,9);});this.settings=this.store.settings();this.audio=new BattlefieldAudio(this.settings);this.clock=new FixedClock();this.ui=new UI(this);this.input=new Input(this.canvas,this.settings,{pause:()=>this.pause(),debug:()=>{this.ui.debug=!this.ui.debug;},lockError:()=>this.lockError()});this.lastFrame=performance.now();this.performance=new PerformanceMonitor();this.metrics=this.performance.metrics;this.frame=0;this.alive=true;this.frameLimiter=new FrameLimiter();this.lastRender=this.lastFrame;this.pendingCpu=0;this.pendingSimulation=0;
  ...
  this.canvas.addEventListener('webglcontextlost',event=>{event.preventDefault();this.disposeMission();this.errorMessage='Utracono kontekst grafiki. Zmniejsz jakość i wczytaj ostatni checkpoint.';this.go('error');});
  this.ui.render('main');if(this.storageWarning)this.ui.toast(this.storageWarning,9);
  const bootGeneration=this.loadGeneration;
  this.store.load().then(snapshot=>{if(bootGeneration!==this.loadGeneration||!this.alive)return;this.checkpoint=snapshot;if(this.state==='main')this.ui.render('main');}).catch(error=>{if(bootGeneration===this.loadGeneration&&this.alive)this.ui.toast(`${error.message} Możesz bezpiecznie rozpocząć misję od nowa.`,9);});
  ...
  const application=this;window.ZiemiaNiczyja={get state(){return application.state;},inspect:()=>this.inspect(),version:'0.4.2'};
 }
 ...
 lockError(){
  ...
  this.pause();this.lockMessage='Nie udało się zablokować kursora. Kliknij przycisk ponownie.';
 }
 ...
 async enter(){
  ...
  try{
   ...
   this.audio.resume(current).catch(e=>{if(current())this.ui.toast(`Dźwięk niedostępny: ${e.message}`,6);});
   await lock;
  ...
  }catch(error){if(current()){this.lockMessage=`${error.message} Kliknij ponownie lub sprawdź uprawnienia strony.`;this.go('ready');}}
  finally{if(sequence===this.enterSequence)this.enterPending=false;}
 }
 async start(snapshot=null){
  const generation=++this.loadGeneration;this.disposeMission(false);this.errorMessage='';this.go('loading');this.canvas.hidden=false;
  ...
  const prepare=async()=>{
   this.ui.progress('Ładowanie lokalnego silnika 3D',.03);
   const [{Simulation},{GameView}]=await this.loadRuntime();if(!current())return;
   this.ui.progress('Budowanie terenu, kolizji i grafu okopów',.12);
  ...
  try{
   await Promise.race([prepare(),new Promise((_,reject)=>{timeout=setTimeout(()=>reject(Error('Przekroczono 45 sekund przygotowania misji. Spróbuj ponownie.')),45000);})]);
  }catch(error){
   if(generation!==this.loadGeneration||!this.alive)return;
   console.error(error);this.disposeMission();this.errorMessage=error.message||String(error);this.go('error');
 ...
 async action(action,button){
  ...
  if(action==='continue'||action==='checkpoint'){
   ...
   catch(e){if(generation===this.loadGeneration&&actionSequence===this.actionSequence){this.errorMessage=e.message;this.go('error');}}return;
  }
  ...
  for(const e of batch){
   ...
   if(e.type==='checkpoint'){
    ...
    try{
     ...
     this.store.save(candidate,{isCurrent:current}).then(persistent=>{if(current())this.ui.toast(persistent?'Zapisano checkpoint w tej przeglądarce.':'Checkpoint zapisany tylko w pamięci tej sesji.',4);}).catch(error=>{if(current())this.ui.toast(`Checkpoint nie został zapisany: ${error.message}`,8);});
    }catch(error){console.error(error);world.director.checkpointPending=true;this.ui.toast(`Checkpoint nie został zapisany: ${error.message}`,8);continue;}
   }
