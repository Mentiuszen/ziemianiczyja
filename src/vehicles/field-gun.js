...
export function createFieldGun(data,terrain){return{...data,crewIds:[...data.crewIds],pos:v3(data.x,terrain.height(data.x,data.z),data.z),yaw:data.yaw,homeYaw:data.yaw,operational:true,disabled:false,reloadLeft:5,ammo:18,recoil:0,lastFire:-100};}
...
export function updateFieldGun(w,g,dt){
 ...
 if(g.disabled||!crew.length){if(g.operational)w.emit('gun-disabled',{id:g.id,pos:{...g.pos}});g.operational=false;return;}
