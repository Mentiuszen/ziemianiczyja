...
function row(key,s){
 ...
 let control='';
 if(d.type==='choice'){
  const option=value=>key==='language'?(value==='en'?'English':'Polski'):t(`${key==='quality'?'quality':'difficulty'}.${value}`);
 ...
 return `<div class="setting"><label id="label-${key}" for="${id}">${label}</label>${control}</div>`;
}
export function settingsScreen(app,tab='gameplay'){
 if(!OPTIONS_TABS.includes(tab))tab='gameplay';
 const s=app.settings;let rows=Object.entries(SETTINGS_SCHEMA).filter(([,d])=>d.category===tab).map(([k])=>row(k,s)).join('');
