...
export class FriendlyMarkerModel {
 ...
 sample(world,view,now,reserved=[]){
  ...
  for(const a of world.npcs){if(a.hp<=0||a.active===false||a.dormant===true||a.faction!==world.player.faction||a.id===world.player.id||distance(a.pos,eye)>HUD.markerRange)continue;
   const head=hitSpheres(a).find(h=>h.part==='head').c,q=view.projectPoint(headAnchor(a));if(!q.visible||reserved.some(r=>q.x>=r.left-16&&q.x<=r.right+16&&q.y>=r.top-9&&q.y<=r.bottom+9))continue;
  ...
  const result=[];for(const marker of visible)if(!result.some(m=>Math.abs(m.x-marker.x)<30&&Math.abs(m.y-marker.y)<18))result.push(marker);return result;
 }
}
export class FriendlyMarkers {
 ...
 update(world,view,settings,reserved){
  ...
  const markers=this.model.sample(world,view,world.time,reserved),live=new Set(markers.map(m=>m.id));
