...
export class CollisionWorld {
 ...
 ground(x,z,fromY=Infinity,r=0){let y=this.terrain.height(x,z);for(const b of this.index.point(x,z,r)){if(b.blocksMovement===false)continue;if(x+r>b.min.x&&x-r<b.max.x&&z+r>b.min.z&&z-r<b.max.z&&b.max.y<=fromY+.49)y=Math.max(y,b.max.y);}return y;}
 ...
 move(actor,dx,dz,dt){const old=actor.pos,h=BODY_HEIGHT[actor.stance]||1.74;let moved=false;const steps=Math.max(1,Math.ceil(Math.max(Math.abs(dx),Math.abs(dz))/.18));
  for(let step=0;step<steps;step++)for(const [x,z] of [[dx/steps,0],[0,dz/steps]]){if(!x&&!z)continue;const p=v3(old.x+x,old.y,old.z+z);if(p.x<MAP.minPlayX||p.x>MAP.maxPlayX||p.z<MAP.minPlayZ||p.z>MAP.maxPlayZ)continue;const ground=this.ground(p.x,p.z,old.y,.29);const rise=ground-old.y;
   if(rise>.46)continue;const horizontal=Math.hypot(x,z),naturalRise=this.terrain.height(p.x,p.z)-this.terrain.height(old.x,old.z);if(actor.grounded&&actor.vy<=0&&naturalRise>Math.max(.025,horizontal*1.08))continue;if(rise>0&&rise<=.46)p.y=ground;
   if(this.canStand(v3(p.x,p.y+.025,p.z),actor.stance,actor.yaw,actor.id)){old.x=p.x;old.z=p.z;if(rise>0)old.y=p.y;moved=true;}
  }
  const floor=this.ground(old.x,old.z,old.y+.03,.29);if(old.y>floor+.025||actor.vy>0){actor.vy=(actor.vy||0)-18*dt;const ny=old.y+actor.vy*dt;
 ...
 ray(o,d,max=180,actors=[],ignoreId=null,options={}){let result=null,best=max;
  for(const b of [...this.index.segment(o,d,max),...this.dynamic]){if(b.blocksRay===false)continue;if(b.id===options.ignoreSolid)continue;const t=rayBox(o,d,b,best);if(t!==null&&t<best){best=t;result={distance:t,solid:b,point:add(o,mul(d,t)),kind:'solid'};}}
