...
export class Player {
 ...
 update(world,dt,input={}){this.health.tick(dt);for(const w of this.weapons)w.tick(dt);this.hurt=Math.max(0,this.hurt-dt);this.hitMarker=Math.max(0,this.hitMarker-dt);this.recoil*=Math.exp(-dt*12);this.meleeLeft=Math.max(0,this.meleeLeft-dt);this.grenadeLeft=Math.max(0,this.grenadeLeft-dt);this.vaultLeft=Math.max(0,this.vaultLeft-dt);
  ...
  if(input.jump&&this.grounded){const d=direction(this.yaw),landing=add(this.pos,mul(d,1.25));landing.y=world.terrain.height(landing.x,landing.z);const obstacle=world.collision.ray(add(this.pos,v3(0,.55,0)),d,1.05);
   if(obstacle?.solid&&obstacle.solid.h<1.1&&!world.collision.overlaps(add(landing,v3(0,.03,0)),1.74)&&Math.abs(landing.y-this.pos.y)<.75){this.vy=4.8;this.vaultLeft=.38;this.stance='stand';}else if(this.stance==='stand')this.vy=4.5;
  }
  ...
  if(input.fire&&!sprint&&this.meleeLeft<=.45&&this.grenadeLeft<=.4){if(this.weapon.fire()){const w=this.weapon;fireBullet(world,this,w,direction(this.yaw,this.pitch),this.ads?w.definition.adsSpread:w.definition.spread+(this.moving?.01:0));this.recoil=w.definition.recoil;this.pitch=clamp(this.pitch-w.definition.recoil*.6,-1.4,1.4);world.stats.playerShots++;}else if(this.weapon.mag===0&&this.weapon.reloadLeft===0&&this.weapon.reload())world.emit('reload',{weapon:this.weapon.id,pos:eye(this)});}
