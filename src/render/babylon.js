...
export async function loadGLB(scene,url,signal){
 const response=await fetch(url,{signal});if(!response.ok)throw Error(`Nie udało się wczytać ${url}: HTTP ${response.status}`);
 const array=await response.arrayBuffer(),view=new DataView(array);if(view.getUint32(0,true)!==0x46546c67||view.getUint32(4,true)!==2)throw Error(`Nieprawidłowy GLB: ${url}`);
 ...
 if(!json||!bin)throw Error(`Niekompletny GLB: ${url}`);
