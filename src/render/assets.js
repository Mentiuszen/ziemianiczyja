...
export class Assets {
 ...
 async load(progress){const names=['british','british-v1','british-v2','german','german-v1','german-v2','smle','gewehr','webley','lewis','hands','mark-iv'];for(let i=0;i<names.length;i++){const name=names[i];progress(`Wczytywanie modeli · ${i+1}/${names.length}`,i/names.length);const c=await loadGLB(this.scene,assetURL(`models/${name}.glb`),this.abort.signal);this.models.set(name,c);for(const m of c.materials){for(const texture of m.getActiveTextures())texture.anisotropicFilteringLevel=2;}}}
