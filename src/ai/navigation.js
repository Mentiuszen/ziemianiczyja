...
export class Navigation {
 ...
 path(from,to){const start=this.nearest(from,true),goal=this.nearest(to);if(!start||!goal)return[];const open=new Heap(),cost=new Float32Array(this.nodes.length);cost.fill(Infinity);const prev=new Int32Array(this.nodes.length);prev.fill(-1);const closed=new Uint8Array(this.nodes.length);cost[start.id]=0;open.push(start.id,0);let count=0,found=false;while(open.length&&count++<12000){const id=open.pop();if(closed[id])continue;if(id===goal.id){found=true;break;}closed[id]=1;for(const e of this.edges(this.nodes[id])){if(this.collision.dynamicBlocked(this.nodes[id].pos,this.nodes[e.id].pos))continue;const g=cost[id]+e.cost;if(g<cost[e.id]){cost[e.id]=g;prev[e.id]=id;open.push(e.id,g+flatDist(this.nodes[e.id].pos,goal.pos));}}}if(!found)return[];const result=[];for(let i=goal.id;i!==start.id&&i>=0;i=prev[i])result.push({...this.nodes[i].pos});result.reverse();if(flatDist(from,start.pos)>.15&&(!result.length||!this.collision.canWalk(from,result[0],.36)))result.unshift({...start.pos});return result;}
 request(actor,target){if(actor.pathPending)return;actor.pathPending=true;this.requests.push({actor,target:{...target}});}
 process(limit=2){while(limit--&&this.requests.length){const {actor,target}=this.requests.shift();actor.pathPending=false;if(actor.hp<=0)continue;actor.path=this.path(actor.pos,target);actor.pathTarget=target;actor.pathIndex=0;}}
 chooseCover(actor,threat){let best=null,score=Infinity;for(const c of this.cover){if(c.owner&&c.owner!==actor.id)continue;const d=flatDist(actor.pos,c.pos);if(d>17||d<.5)continue;const hidden=!this.collision.visible(v3(c.pos.x,c.pos.y+.8,c.pos.z),v3(threat.x,threat.y+1.4,threat.z));if(!hidden)continue;const s=d+flatDist(c.pos,threat)*.04;if(s<score){const route=this.path(actor.pos,c.pos);if(route.length){best=c;score=s;}}}if(best){this.releaseCover(actor.id);best.owner=actor.id;}return best;}
 releaseCover(id){for(const c of this.cover)if(c.owner===id)c.owner=null;}
}
