...
function squadGoal(w,n){const phase=w.director.phase;if(n.faction==='uk'){
 ...
 if(phase===3)return v3(-27+offset,0,72+(i%3)*2);
 ...
 if(n.group==='village'&&phase>=4)return v3(n.pos.x>4?17:-9,0,166);
 return v3(n.x,0,n.z);
}
function think(w,n){
 ...
 if(target){if(n.targetId!==target.id){n.reaction=w.time+(.5+w.random()*.48)*(n.faction==='de'?w.difficulty.reaction:1);n.targetId=target.id;}
  ...
  if(n.grenades>0&&n.grenadeCooldown<=0&&!n.fixed&&flatDist(n.pos,target.pos)<21&&flatDist(n.pos,target.pos)>8&&w.actors.every(a=>a.faction!==n.faction||a.hp<=0||flatDist(a.pos,target.pos)>7)){
  ...
  for(const sound of w.noises){if(sound.faction!==n.faction&&flatDist(n.pos,sound.pos)<35){n.lastKnown=v3(sound.pos.x+(w.random()-.5)*8,sound.pos.y,sound.pos.z+(w.random()-.5)*8);n.lastSeen=w.time-2;n.state='search';if(!n.fixed)requestMove(w,n,n.lastKnown);return;}}
...
export function updateSoldier(w,n,dt){
 ...
 if(target&&['aim','fire','reload'].includes(n.state)){
  const d=sub(eye(target),eye(n)),heading=Math.atan2(d.x,d.z);n.yaw=approachAngle(n.yaw,heading,dt*2.6);n.pitch=-Math.atan2(d.y,Math.hypot(d.x,d.z));
  const aligned=Math.abs(angleDiff(n.yaw,heading))<.15;
  if(aligned&&w.time>=n.reaction&&n.weapon.cooldown<=0&&!n.weapon.reloadLeft&&w.collision.visible(eye(n),eye(target))){
   ...
   if(n.weapon.fire()){n.state='fire';n.shotTime=w.time;const base=n.weapon.id==='mg08'?.067:n.faction==='uk'?.035:.047;fireBullet(w,n,n.weapon,norm(d),base*(n.faction==='de'?w.difficulty.spread:1)*(1+n.suppression*.35));}
  }
 }
