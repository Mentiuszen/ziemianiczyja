...
PROJECT=Path(__file__).resolve().parents[1];ROOT=PROJECT/'dist' if args.dist else PROJECT;OUT=PROJECT/'docs/v0.3-tests/lifecycle';OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 browser=p.chromium.launch(executable_path=shutil.which('chromium'),headless=True,args=['--no-sandbox','--use-angle=swiftshader','--enable-unsafe-swiftshader'])
 report['browser']=browser.version;page=browser.new_page(viewport={'width':1280,'height':800});requests=[];fail_asset=False
 ...
 for i in range(args.cycles):
  ...
  if i==0:
   page.keyboard.press('KeyE');page.wait_for_timeout(650);page.mouse.click(640,400);page.wait_for_timeout(350);assert page.evaluate('__ZN_TEST__.app.world.player.weapon.mag')==9
   page.keyboard.press('KeyC');page.wait_for_timeout(250);assert page.evaluate('__ZN_TEST__.app.world.player.stance')=='crouch';page.keyboard.press('KeyC');page.keyboard.press('KeyG');page.wait_for_timeout(300);assert page.evaluate('__ZN_TEST__.app.world.player.grenades')==2;report['pointerLockAndInputs']=True
