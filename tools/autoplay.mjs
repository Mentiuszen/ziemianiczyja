import {BRIEFING_DOOR} from '../src/world/briefing-door.js';
...
const s=new Simulation(null,'recruit');if(process.argv.includes('--destroyed-tanks')){for(const t of s.tanks){t.state='destroyed';t.integrity=0;t.gunWorking=false;}console.log('Fallback fixture: both tanks destroyed before mission; no player position/health edits.');}let phase=-1,path=[],index=0,stuck=0,last={...s.player.pos},checkpoints=[],nextRouteRetry=0;
for(let step=0;step<36000&&s.player.hp>0&&s.director.phase<7;step++){
 const p=s.player,o=s.director.objective,input={};
 if(phase!==s.director.phase){phase=s.director.phase;const goal=phase===0?{x:BRIEFING_DOOR.x-1.2,z:BRIEFING_DOOR.z}:phase===2?{x:23,z:62}:phase===4?{x:36,z:113}:phase===6?{x:4,z:160}:{x:o.x,z:o.z};path=s.nav.path(p.pos,goal);index=0;console.log('phase',phase,o.id,'time',s.time.toFixed(1),'HP',p.hp.toFixed(1),'route',path.length);}
 ...
 else if(phase===0){input.lookX=angleDiff(Math.PI/2,p.yaw);input.lookY=-p.pitch;input.interact=true;}
 else if(phase===2||phase===3||phase===4||phase===5)input.interact=true;
 // A phase change can precede completion of the physical door animation.
 // Retry a temporarily unavailable route, as a human would wait for the leaves to clear.
 if(!path.length&&phase>0&&phase<6&&s.time>=nextRouteRetry){nextRouteRetry=s.time+.75;const goal=phase===2?{x:23,z:62}:phase===4?{x:36,z:113}:{x:o.x,z:o.z};path=s.nav.path(p.pos,goal);index=0;}
 ...
 if(flatDist(last,p.pos)<.002&&index<path.length)stuck+=1/60;else stuck=0;last={...p.pos};if(stuck>3){path=s.nav.path(p.pos,phase===0?{x:BRIEFING_DOOR.x-1.2,z:BRIEFING_DOOR.z}:phase===2?{x:23,z:62}:{x:o.x,z:o.z});index=0;stuck=0;}
}
