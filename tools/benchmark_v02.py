...
xvfb-run -a python tools/benchmark_v02.py --baseline /path/to/ziemia-niczyja-v0.1.0
Uses real vendored engines and assets, identical deterministic simulation inputs,
...
PROJECT=Path(__file__).resolve().parents[1];OUT=PROJECT/'docs/v0.2-tests';base='http://local.test/'
report={'viewport':[1280,720],'quality':'medium','renderScale':1,'samples':args.samples,'warmup':10,'renderer':'Chromium / ANGLE SwiftShader (software)','platform':platform.platform(),'method':'Identical simulation seed and 1/60 s simulation tick per measured frame; GPU/compositor paced requestAnimationFrame. CPU = tick + render submission, excludes HUD. Not a 60 FPS gameplay claim. Per-frame draw-counter differences, not accumulated engine counters.','runs':[]}
with sync_playwright() as p:
 for scene_name in ['trench-start','battle-25-seconds']:
  for label,root in [('v0.1',args.baseline),('v0.2',PROJECT)]:
