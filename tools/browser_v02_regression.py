"""v0.2 real-Chromium regressions. Run: xvfb-run -a python tools/browser_v02_regression.py
...
PROJECT=Path(__file__).resolve().parents[1];ROOT=PROJECT/'dist' if args.dist else PROJECT;OUT=PROJECT/'docs/v0.2-tests';OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
 ...
 page.set_viewport_size({'width':1280,'height':720});page.screenshot(path=str(OUT/'menu-v02.png'))
 ...
 report['sky']=sky
 report['qualities']=[]
 for quality in ['medium','high','low']:
  result=page.evaluate("""async quality => {const a=__ZN_TEST__.app,v=a.view;a.world.player.pitch=-.05;a.world.player.yaw=0;v.applySettings({...a.settings,quality});for(let i=0;i<4;i++){v.render(false);await new Promise(requestAnimationFrame);}return {quality:v.quality,shadow:!!v.shadow,shadowSize:v.shadow?.getShadowMap().getSize().width||0,npcs:a.world.npcs.length,lodCounts:v.renderStats.lodCounts,activeMeshes:v.renderStats.activeMeshes};}""",quality)
  assert result['npcs']==28 and sum(result['lodCounts'])==28,result
  assert result['shadowSize']=={'low':0,'medium':1024,'high':2048}[quality],result
